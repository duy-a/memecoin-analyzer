...
function buildFibLevelDetails(fib, fibSet, price) {
  ...
  return orderedLevels.map((level) => {
    ...
    if (priceValue !== null && Number.isFinite(fibPrice)) {
      if (numericLevel === 1) isValid = priceValue <= fibPrice;
      else if (numericLevel === 0) isValid = priceValue >= fibPrice;
      else if (numericLevel >= 0.5) isValid = priceValue >= fibPrice;
      else isValid = priceValue <= fibPrice;
    }
